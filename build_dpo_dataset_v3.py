...
DEFAULT_OUTPUT = "/scratch/xla2767/hold2/data/nlp/hf_cot_dpo_v3"
PAIR_KEEP_PROB = 0.5
...
def _make_overly_concise(factor: str, label: str) -> str:
    reasoning = (
        f"For {factor}, the filing evidence roughly points {label}. "
        "That is the main takeaway."
    )
    return reasoning


def _build_candidates_v3(row: pd.Series) -> dict[str, str]:
    ...
    overly_concise_reasoning = _make_overly_concise(factor, teacher_label)
    ...
    return {
        ...
        "overly_concise": _format_thinking_response(
            reasoning=overly_concise_reasoning,
            label=teacher_label,
            confidence=max(0.35, min(0.75, teacher_conf - 0.10)),
        ),
...
def choose_rows_v3(row: pd.Series, teacher_tolerance: int) -> list[dict[str, str]]:
    candidates = _build_candidates_v3(row)
    ...
    rows.append(
        {
            ...
            "preference_source": "teacher_balanced_over_verbose_rambling",
        }
    )
    ...
    rows.append(
        {
            "chosen": candidates["teacher_balanced"],
            "rejected": candidates["overly_concise"],
            "preference_source": "teacher_balanced_over_overly_concise",
        }
    )
    ...
    if aligned:
        rows.append(
            {
                "chosen": candidates["teacher_balanced"],
                "rejected": candidates["teacher_concise"],
                "preference_source": "teacher_balanced_over_teacher_concise",
            }
        )
    else:
        rows.append(
            {
                ...
                "rejected": candidates["verbose_rambling"],
                "preference_source": "return_aligned_balanced_over_verbose_rambling",
            }
        )
        rows.append(
            {
                "chosen": candidates["return_aligned_balanced"],
                "rejected": candidates["overly_concise"],
                "preference_source": "return_aligned_balanced_over_overly_concise",
            }
        )
        if row.get("evidence_count", 0) >= 2:
            rows.append(
                {
                    "chosen": candidates["return_aligned_balanced"],
                    "rejected": candidates["conservative"],
                    "preference_source": "return_aligned_balanced_over_conservative",
                }
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build DPO v3 pairs with anti-verbose and anti-overly-concise preference.")
    ...
    parser.add_argument("--pair-keep-prob", type=float, default=PAIR_KEEP_PROB)
...
def main() -> None:
    ...
    if df.empty:
        raise RuntimeError("No eligible task13 rows found for DPO v3 dataset construction.")
    ...
    for row_idx, (_, row) in enumerate(df.iterrows()):
        pair_rows = choose_rows_v3(row, args.teacher_tolerance)
        base = row.to_dict()
        for pair in pair_rows:
            source = pair["preference_source"]
            slot = (hash((base["ticker"], base["factor"], base["report_date"], source, row_idx)) % 1000) / 1000.0
            if slot >= args.pair_keep_prob:
                continue
            out = {
                "prompt": base["prompt"],
                "chosen": pair["chosen"],
                ...
                "parsed_report_date": base["parsed_report_date"],
                "year": base["year"],
                "factor": base["factor"],
                "sector": base.get("sector"),
                "teacher_label": base["teacher_label"],
                "return_label": base["return_label"],
                "teacher_confidence": base["teacher_confidence"],
                "ret_1m": base["ret_1m"],
                "excess_1m": base["excess_1m"],
                "evidence_count": base["evidence_count"],
                "historical_count": base["historical_count"],
    ...
    for split, part in out_df.groupby("split"):
    ...
    logging.info(
        "Saved DPO v3 dataset to %s | train=%d validation=%d test=%d",
        output_dir,
    ...
    if split_rows["train"]:
        logging.info("Sample preference source: %s", split_rows["train"][0]["preference_source"])
